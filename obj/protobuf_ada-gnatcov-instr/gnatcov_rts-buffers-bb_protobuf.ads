...
package GNATcov_RTS.Buffers.BB_protobuf is

   pragma Preelaborate;

   Statement_Buffer : Coverage_Buffer_Type (0 .. 316) := (others => False);
   Statement_Buffer_Address : constant System.Address := Statement_Buffer'Address;
   pragma Export (C, Statement_Buffer_Address, "xcov__buf_stmt__B_protobuf");
   ...
   pragma Export (C, Decision_Buffer_Address, "xcov__buf_dc__B_protobuf");

   MCDC_Buffer : Coverage_Buffer_Type (0 .. -1) := (others => False);
   MCDC_Buffer_Address : constant System.Address := MCDC_Buffer'Address;
   pragma Export (C, MCDC_Buffer_Address, "xcov__buf_mcdc__B_protobuf");

   Buffers : aliased Unit_Coverage_Buffers :=
     (Unit_Name_Length => 8,
      Project_Name_Length => 0,
      Fingerprint => (25, 100, 138, 164, 57, 165, 173, 227, 93, 98, 75, 131, 220, 56, 38, 127, 105, 85, 110, 235),
      ...
      Statement_Last_Bit => 316,
      Decision_Last_Bit => -1,
      MCDC_Last_Bit => -1);

end GNATcov_RTS.Buffers.BB_protobuf;
